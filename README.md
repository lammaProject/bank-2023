...
## Запуск проекта
1. Для запуска достаточно написать в консоли `./start.sh` - запускает сразу и client и server.
2. Для запуска client надо написать `npm run dev`.
3. Для запуска server `npm start`.
...
2. Добавил удаление карточек, все кроме основной можно удалить. Что бы вызвать удаление нужно подержать по карточке. 
